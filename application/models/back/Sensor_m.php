...
class sensor_m extends My_Model{
	...
	function makemqtt($data){
		$url = $this->config->item('url_node')."mqtt/";				
		return json_decode($this->sendPost($url,$data));
	}

	function updatemqtt($id,$data){
		$url = $this->config->item('url_node')."mqtt/edit/".$id;				
		return json_decode($this->sendPost($url,$data));
	}

	function search_mqtt($data){
		$url = $this->config->item('url_node')."mqtt/search/";				
		return json_decode($this->sendPost($url,$data));
	}
}
